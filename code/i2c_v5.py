		...
		if (t[0]==0x36):			# MtData2 is MID 0x36, we need it
			if (write == 1): 
		        	...
		        	print("\r\nWe have MtData2 message")
                		#zf=open("/home/pi/data("+ts+").txt", 'a')	# open file to save values
			...
			while (k<t[1]): # read byte after byte if it more than lenght in message
				...
				if (xdi == 0x1020):   	#  0x1020 = PacketCounter
					...
					csvQ=str(d1)+str(d2)+str(d3)+str(d4);
					csvQ=csvQ.replace("(","")
					csvQ=csvQ.replace(")","")
					k=k+17
...
while True:
	if (GPIO.input(29) == GPIO.HIGH):
			...
			ts_temp=str(datetime.now());
			ts4file=ts_temp.replace(" ", "_");
                        zf=open("/home/pi/data_"+ts4file+".csv", 'a')
                        zf.write("Date Time,,Sensor1,,,,,,,,,,,,,,Sensor2\r\n")
			zf.write(",,Acc,,,Gy,,,Mag,,,Qua,,,,,Acc,,,Gy,,,Mag,,,Qua,,,,\r\n");
			zf.write(",,X,Y,Z,X,Y,Z,X,Y,Z,A,B,C,D,,X,Y,Z,X,Y,Z,X,Y,Z,A,B,C,D\r\n");
			zf.close()
        	else:
            		print("-------------------STOP MODE-------------------")
            		need_wr = 0;
	...
	time.sleep(0.1) # we read PipeStatus 5 time in second
    	s1=sensor_get_measure(dev1, "Sensor1 ", need_wr)
    	time.sleep(0.1)
    	s2=sensor_get_measure(dev2, "Sensor2 ", need_wr)
	if (len(s1)>0  and len(s2)>0):
		if (need_wr>0):
			zf=open("/home/pi/data_"+ts4file+".csv", 'a')
			zf.write(str(datetime.now())+",,"+s1+","+s2+"\r\n")
			zf.close();
	if (GPIO.input(38) == GPIO.HIGH):
		time_for_power_off=time_for_power_off-1;
		print("time_for_power_off = "+str(time_for_power_off))
		if (time_for_power_off <= 0):
			print("-------------------POWER OFF-------------------")
			GPIO.output(40, GPIO.LOW)
			GPIO.output(31, GPIO.LOW)
			time.sleep(1)
			call("sudo nohup shutdown +0", shell=True)
			break;
	else:
		time_for_power_off=15;
dev1.close()
