# I2C communication between pi and xsens & measuring live motion data 
#
# References:
# https://cdn2.hubspot.net/hubfs/3446270/Downloads/Manuals/Hardware_Integration_Manual_MTi_1-series.pdf
# https://cdn2.hubspot.net/hubfs/3446270/Downloads/Leaflets/MTi-1-series-datasheet.pdf
# https://1drv.ms/b/s!AqtCaRJTR76vymVlSyjQYkbMKXnQ?e=ob6zoP
# https://base.xsens.com/hc/en-us/articles/115002856865-Best-practices-I2C-and-SPI-for-MTi-1-series?mobile_site=true
# https://1drv.ms/b/s!AqtCaRJTR76vsw1RJycZP2z9RCGO?e=eQnXgK 
#
...
OPCODE_CONTROLPIPE      =   0x03;
OPCODE_PIPESTATUS       =   0x04;
OPCODE_NOTIFICATIONPIPE =   0x05;
OPCODE_MEASUREMENTPIPE  =   0x06;
...
for i in range(300): 	   	# in this loop we must read notifications from sensor. when we catch 62 (0x3E) (WakeUp) - it means that sensor was reset and boot
	...
	try:
		...
		t = bytearray()		# create new empy array of bytes
       		t.extend(map(ord, s)) # S is string, but we need bytes
		...
		dev.write([5])		# we work with NotificationPipe(5) (2nd link page 11)
		...
		if (t[0]==62):		# if it 0x3E WakeUp	then goto from this loop
			...
			break;
		#dev.write([OPCODE_CONTROLPIPE ,63,0,194]) # wakeupAck = 63 (0x3F)
		dev.write([6])		# we work with MeasurementPipe(6) (2nd link page 11)
...
# set configuration for sensor: Acc, Gyr, Mag and Quaternion refresh 1 time in 1 second
dev.write([3,192,28,0x10,0x20,0xff,0xff,0x10,0x60,0xff,0xff,0x40,0x20,0,1,0x80,0x20,0,1,0xc0,0x20,0,1,0x20,0x10,0,1,0xe0,0x20,0xff,0xff,119]) #172
time.sleep(0.1)
...
while True:
	...
	pipem_len=0
	try:
		dev.write([4])		# we work with PipeStatus(4) to know how many bytes we must read (2nd link page 11)
		s = dev.read(4)		# now we read 4 bytes to know lenght of massages.
		t = bytearray()
		t.extend(map(ord, s))
	...
	if (pipem_len>0):
		...
		while (k<t[1]): # read byte after byte if it more than lenght in message
			...
			if (xdi == 0x1020):   	#  0x1020 = PacketCounter
				...
				print("Quaternion: "+str(d1)+" "+str(d2)+" "+str(d3)+" "+str(d4)) # print to output
                zf.write("Quaternion: "+str(d1)+"   "+str(d2)+"   "+str(d3)+" "+str(d4)+"\r\n")       # print to file
				k=k+17
